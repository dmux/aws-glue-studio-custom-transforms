"""
case_transform provides transformation logic for AWS Glue Studio to transform the casing of a column
...
def transform(df: DataFrame, column_name: str, case: str) -> DataFrame:
    ...
    Args:
        ...
        case (str): Determines if you will transform to lowercase, uppercase, titlecase, or propercase.
    ...
    """
    valid_cases = ["uppercase", "lowercase", "titlecase", "propercase"]
    if case not in valid_cases:
        raise ValueError(
            f"Provided value '{case}' for parameter 'case' is invalid. Valid options are: {', '.join(valid_cases)}."
        )
    ...
    if case == "propercase":
        df = df.withColumn(column_name, F.regexp_replace(F.initcap(F.col(column_name)), " ", ""))
...
def case_transform(self, column_name: str, case: str) -> DynamicFrame:
    """
    case_transform will take your specified column name and either transform it to
    all uppercase, all lowercase, titlecase, or propercase.
